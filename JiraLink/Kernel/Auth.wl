...
(* Secure storage will not accept a bare association unless one of its fields is
   literally called "Password". Anything else -- a personal access token, for
   instance -- has to be wrapped in SystemCredentialData, whose second argument
   names the field holding the secret. Storing a bare association fails outright
   with SystemCredentialData::nsec and writes nothing.

   The value read back is the SystemCredentialData expression, not the
   association that went in, so it has to be unwrapped again here. *)

jiraSecretField[cred_Association] := Which[
  StringQ[Lookup[cred, "Token", None]],    "Token",
  StringQ[Lookup[cred, "Password", None]], "Password",
  True,                                    None
];

jiraReadStoredCredential[key_String] := Module[{v},
  v = Quiet @ SystemCredential[key];
  v = Replace[v, SystemCredentialData[a_Association, ___] :> a];
  If[AssociationQ[v], v, None]
];

JiraStoreCredential[url_String, cred_Association] := Module[{norm, key, field, data, res},
  norm = jiraNormalizeURL[url];
  If[norm === $Failed, Return[$Failed]];

  field = jiraSecretField[cred];
  If[field === None,
    Message[JiraLink::nosecret];
    Return[$Failed]
  ];

  key = jiraCredentialKey[norm];

  (* Secure storage requires every key and value to be a string, so anything
     else is dropped rather than allowed to fail the whole write. *)
  data = Select[Join[<|"BaseURL" -> url|>, cred], StringQ];

  res = Quiet @ Check[
    SystemCredential[key] = SystemCredentialData[data, field],
    $Failed,
    {SystemCredentialData::nsec}
  ];

  (* Report the failure rather than handing back a key that stores nothing. *)
  If[res === $Failed,
    Message[JiraLink::storefail, key];
    Return[$Failed]
  ];
