...
jiraCredentialKey        = Symbol[$p <> "jiraCredentialKey"];
jiraNormalizeURL         = Symbol[$p <> "jiraNormalizeURL"];
jiraReadStoredCredential = Symbol[$p <> "jiraReadStoredCredential"];
...
(* Secure storage round trip
   ...
   Secure storage rejects a bare association whose secret field is not called
   "Password": the write fails with SystemCredentialData::nsec and stores
   nothing, while the value read back afterwards is a SystemCredentialData
   expression rather than the association that went in. Both directions are
   pinned here, against a throwaway host, and cleaned up afterwards.

   This touches the operating system's credential store, unlike every other test
   in this file. *)

VerificationTest[
  Module[{u = "https://jiralink-selftest.invalid/jira", stored, got, key},
    JiraDeleteCredential[u];
    stored = JiraStoreCredential[u, <|"Token" -> "round-trip-token"|>];
    key = jiraCredentialKey[jiraNormalizeURL[u]];
    got = If[StringQ[stored], jiraReadStoredCredential[key], None];
    JiraDeleteCredential[u];
    {StringQ[stored], Lookup[If[AssociationQ[got], got, <||>], "Token", None]}
  ],
  {True, "round-trip-token"},
  TestID -> "token-survives-secure-storage-round-trip"
];

VerificationTest[
  Module[{u = "https://jiralink-selftest.invalid/jira", stored, got, key},
    JiraDeleteCredential[u];
    stored = JiraStoreCredential[u, <|"Username" -> "fred", "Password" -> "pw"|>];
    key = jiraCredentialKey[jiraNormalizeURL[u]];
    got = If[StringQ[stored], jiraReadStoredCredential[key], None];
    JiraDeleteCredential[u];
    Lookup[If[AssociationQ[got], got, <||>], "Password", None]
  ],
  "pw",
  TestID -> "basic-credential-survives-round-trip"
];

VerificationTest[
  (* A credential with no secret field must fail loudly, not claim success. *)
  Quiet @ JiraStoreCredential["https://jiralink-selftest.invalid/jira", <|"BaseURL" -> "x"|>],
  $Failed,
  TestID -> "credential-without-secret-field-fails"
];
